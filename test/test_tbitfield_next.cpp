...
TEST(TBitField, can_set_last_bit_1)
{
    ...
    const size_t n = sizeof(unsigned int) * 8 * num_el;
    TBitField field(n);
    ASSERT_EQ(field.getNumBytes(), sizeof(unsigned int) * num_el);
...
TEST(TBitField, can_set_last_bit_3)
{
    ...
    ASSERT_EQ(field2.getNumBytes(), sizeof(unsigned int) * num_el);
...
{
    const size_t num_el = 1;
    const size_t n = sizeof(unsigned int) * num_el;
    TBitField field(n);
