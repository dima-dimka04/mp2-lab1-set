...
private:
    ...
    myuint* pMem;     // память для представления битового поля
    size_t memLen;   // к-во эл-тов mymyuint для представления бит.поля //длина массива pMem

    // методы реализации
    size_t getIndex(const size_t n) const; // индекс в pМем для бита n
    myuint getMask(const size_t n) const;    // битовая маска для бита n
public:
...
//   Структура хранения битовогo поля
//   бит.поле - набор битов с номерами от 0 до bitLen
//   массив pМем рассматривается как последовательность mymyuint элементов
